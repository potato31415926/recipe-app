...
public class SearchListAdapter extends NestedRecyclerView.Adapter<SearchListAdapter.ViewHolder> {

    private final List<Recipe> Data;

    public SearchListAdapter(List<Recipe> data) {
        Data = data;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        Glide.with(holder.itemView.getContext())
                ...
                .into(holder.image);
        holder.tags.setText(truncateString(Data.get(position).getTag()));
        holder.name.setText((Data.get(position).getName()));
        holder.constraintLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(v.getContext(), DetailActivity.class);
                intent.putExtra("content", JSONObject.toJSONString(Data.get(position)));

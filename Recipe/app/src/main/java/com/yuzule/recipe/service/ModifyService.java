...
public class ModifyService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        User user = JSONObject.parseObject(intent.getStringExtra("user"), User.class);
        // 访问接口并获取数据
        modify(user);
        ...
        api.modify(user)
                ...
                .subscribe(new Observer<Information>() {
                    ...
                    public void onNext(Information message) {
                        ...
                        broadcastIntent.setAction("com.yzl.broadcast.modify");
                        broadcastIntent.putExtra("message", message.getMessage());
                        broadcastIntent.putExtra("user", JSONObject.toJSONString(user));
                        // 发送Broadcast

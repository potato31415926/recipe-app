...
public class CollectService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // 访问接口并获取数据
        collect(user_id, content_id, content);
        return super.onStartCommand(intent, flags, startId);
    }

    private void collect(int user_id, int content_id, String content) {
        CollectRequest collectRequest = new CollectRequest();
        collectRequest.setUser_id(user_id);
        collectRequest.setContent_id(content_id);
        collectRequest.setContent(content);
        api = ApiClient.getRetrofit().create(Api.class);
        api.collect(collectRequest)
                ...
                .subscribe(new Observer<Information>() {
                    ...
                    public void onNext(Information information) {
                        ...
                        broadcastIntent.setAction("com.yzl.broadcast.collect");
